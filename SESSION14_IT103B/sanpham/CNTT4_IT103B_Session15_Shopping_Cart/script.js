...
const renderCarts = (carts) => {
  if (Array.isArray(carts)) {
    ...
    const elementMaps = carts.map((element, index) => {
...
const handleDecreasement = (index) => {
  // lấy số lượng của sản phẩm trong giỏ hàng theo vtri
  const productQuantity = carts[index].quantity;

  if (productQuantity === 1) {
    handleRemoveCart(carts[index].id);
  } else {
    carts[index].quantity -= 1;
  }
};
